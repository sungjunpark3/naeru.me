...
FADE            = 26
DILATE_PX       = 24
# **덮개에 바닥선을 두지 말 것.** 2026-09-08에 접지부 잔디를 살리려고 덮개를
# 캐릭터 바닥선 위로 제한했는데, 그러면 plate가 발을 못 지워서 **발이 배경
# 정지이미지에 구워진다.** 폴짝 뛰면 바닥에 발 한 쌍이 남아 있는 게 보인다
# (제보: "점프할때 다리가 잘리잖아, 바닥에 다리가 남아").
# 덮개는 캐릭터를 남김없이 덮어야 하고, 접지부 잔디는 **matte.py의
# GROUND_LINE**(알파 상한)으로 잘라낸다.
