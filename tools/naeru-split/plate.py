...
FADE            = 26
DILATE_PX       = 24
# 접지부 바닥선 (CROP 좌표). 덮개가 이 선보다 GROUND_PAD 아래로는 안 내려간다.
#
# 왜 필요한가 — 덮개가 분홍기 union + 24px 팽창이라 **캐릭터 밑의 밝은 잔디
# 하이라이트까지 덮어** 확산으로 뭉개 놓는다. 그러면 그 자리 plate가 실제보다
# 20~50 어두워지고, matte.py의 차이 키가 그 밝은 잔디를 캐릭터로 오인한다
# (2026-09-07 제보: 발 옆 흰 덩어리). 자동으로는 못 가른다 — 차이·분홍기·
# 초록기·시간축 σ·연결성분 전부 잔디와 발가락이 겹친다. 그래서 316프레임
# 시간축 최소(캐릭터가 한 번이라도 지나간 자리)를 6배로 펴서 손으로 그었다.
# 발가락 두 무리(x166~188, x292~334)에서 아래로 파고드는 게 맞다.
# **시간축 최소 지도로 그으면 안 된다** — 접지 그림자까지 어둡게 나와서 배 밑
# 선을 9px 낮게 잡게 되고, 그러면 문제의 밝은 띠가 그대로 선 위에 남는다.
# 열별 스캔라인에서 어두운 밑동이 끝나는 y를 읽는 게 맞다(x240→367,
# x265→363, x285→363). 잔디와 발가락이 **대각선으로 붙어 있는 곳**(왼쪽 발)은
# R-G 지도로 갈랐다 — 분홍 몸이 30~45, 베이지 잔디가 2~20이다. 잔디 쐐기
# (x148~166)와 흰 발가락(x168~184)이 이웃한 열이라 x166→x168에서 366→384로
# 급하게 떨어뜨려야 둘 다 맞는다.
# (이전 메모) x160은 y352~358이 캐릭터
# (L 66~73)이고 y362~372가 밝은 띠(L 129~142)라 선이 360이어야 한다.
GROUND_LINE     = [(0, 352), (138, 352), (144, 358), (150, 359), (156, 361),
                   ...
                   (420, 356), (575, 356)]
GROUND_PAD      = 1
GROUND_SOFT     = 2      # 그 아래로 0까지 떨어지는 램프
...
def build_crop_mask():
    ...
    # 바닥선 아래는 덮지 않는다 (**블러 뒤에** 걸어야 한다) — 원본 잔디를 그대로 남겨야 matte의 차이 키가
    # 거기서 0이 된다(위 GROUND_LINE 주석)
    ox_w = CROP_ORIGIN[0] - WORK_ORIGIN[0]
    oy_w = CROP_ORIGIN[1] - WORK_ORIGIN[1]
    xs = [x + ox_w for x, _ in GROUND_LINE]
    ys = [y + oy_w for _, y in GROUND_LINE]
    limit = np.interp(np.arange(WORK_SIZE[0]), xs, ys) + GROUND_PAD
    rows = np.arange(WORK_SIZE[1])[:, None]
    keep = np.clip((limit[None, :] + GROUND_SOFT - rows) / GROUND_SOFT, 0, 1)
    final_mask = ImageChops.multiply(
        final_mask, Image.fromarray((keep * 255).astype(np.uint8)))
